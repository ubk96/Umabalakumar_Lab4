...
where `order`.pricing_id=supplier_pricing.pricing_id and supplier_pricing.pro_id=product.pro_id;
...
from (select supplier_pricing.supp_id,test2.ORD_ID , test2.RAT_RATSTARS from supplier_pricing inner join 
(select `order`.pricing_id,rating.ORD_ID, rating.RAT_RATSTARS from `order` inner join rating 
on rating.`ord_id`=`order`.ord_id) as test2 on test.pricing_id = supplier_pricing.pricing_id)
